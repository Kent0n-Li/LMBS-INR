...
def evaluate_cubic_bspline(
    ...
    if transpose:
        ...
        #print("kernel shapes:", [k.shape for k in kernel])
        #print("stride:", stride)
        ...
        if shape is not None:
            ...
            #output = output[ (slice(0, N), slice(0, C)) + tuple(slice(s, s + n) for s, n in zip(stride, shape)) ]
    ...
    # Implementation adapted from MIRTK
        ...
        #print("output 6", output.shape)
        #print("refer shape", shape)
        if shape is not None:
            output = output[(slice(0, N), slice(0, C)) + tuple(slice(0, n) for n in shape)]
        #print("output 7", output.shape)
    return output
